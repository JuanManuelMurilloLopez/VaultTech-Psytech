const colores = [
  ...
  terminarBtn.addEventListener("click", async (e) => {
    ...
    // Checa que JSON esta bien enviado
    console.log("Datos que se están enviando:", selecciones); // Checa si los datos son correctos
    ...
    try {
        const response = await fetch("/aspirante/guardar-selecciones-colores", {
            ...
            body: JSON.stringify({ selecciones }) // Enviar datos en JSON
        });

        const resultado = await response.json();
        if (response.ok) {
            alert("¡Selecciones guardadas correctamente!");
            window.location.href = "/aspirante/respuestas-enviadas";
        } else {
